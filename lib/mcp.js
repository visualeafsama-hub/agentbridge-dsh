...
export class McpServer {
  constructor({ mode = "channel", engine, log = console.error } = {}) {
    this.mode = mode;
    this.engine = engine; // ChannelClient (channel), ControlClient (attach), DshAppServer (server) or StandbyEngine (standby)
    this.log = log;
    this.initialized = false;
  }

  peerLabel() {
    if (this.mode === "channel") return "codex side (through the agent-bridge daemon)";
    if (this.mode === "attach") return "codex side (attached)";
    if (this.mode === "standby") return "pair peer (daemon not running yet — tools auto-reconnect)";
    return "claude side (as the pair's codex app-server)";
  }
  ...
  toolDefs() {
    return [
      {
        name: "send_message",
        description: `Send a text message to the ${this.peerLabel()} and wait for the turn to complete.`,
        inputSchema: {
          type: "object",
          properties: {
      ...
      {
        name: "get_messages",
        description: `Return any messages received from the ${this.peerLabel()} that have not been consumed yet.`,
        inputSchema: {
          type: "object",
          properties: {
    ...
    switch (name) {
      case "send_message": {
        ...
        if (this.mode === "standby") {
          // Lazy reconnect: works once the peer daemon is up, errors otherwise.
          const msg = await this.engine.sendMessage(text, { timeoutMs });
          return this._toolResult(JSON.stringify({ ok: true, threadId: msg.threadId, turnId: msg.turnId, reply: msg.text }));
        }
      ...
      case "get_messages": {
        ...
        if (this.mode === "standby") {
          ...
          return this._toolResult(JSON.stringify({ messages: this.engine.drainInbound() }));
        }
